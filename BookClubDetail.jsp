	...
	<main class="DetailBCMain">
		...
		<section class="contDBCM">
			...
			<article class="contDBCM-03">
                <%
                    // 댓글 정보 가져오기
                    String commentQuery = "SELECT commentId, content, userId, created_at FROM clubcomment WHERE postId = ?";
                    PreparedStatement pstmtComment = con.prepareStatement(commentQuery, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
                    pstmtComment.setInt(1, postId);
                    ResultSet rsComment = pstmtComment.executeQuery();
                    
                    // 댓글 수 카운트
                    rsComment.last();
                    int commentCount = rsComment.getRow();
                ...
                <ul>
                    <%
                        while (rsComment.next()) {
                            int commentId = rsComment.getInt("commentId");
                            String commentContent = rsComment.getString("content");
                            String commentUserId = rsComment.getString("userId");
                            Timestamp commentCreatedAt = rsComment.getTimestamp("created_at");
                            String formattedCommentDate = sdf.format(commentCreatedAt);
                    %>
                    <li>
                        <img src="images/mypageBC.png">
                    </li>
<li class="Content">
    <p><%=commentUserId%></p>
    <div id="commentContent<%=commentId%>"><%=commentContent%></div>
    <p><%=formattedCommentDate%></p>
    <div style="display: flex;">
        <!-- 수정 버튼을 누르면 editComment 함수 호출 -->
        <button type="button" onclick="editComment(<%=commentId%>)">수정</button>
        <!-- 삭제 버튼 -->
        <form method="post" action="CommentDelete.jsp" name="del" onsubmit="return confirmDelete()">
...
<script>
    function editComment(commentId) {
        // 해당 댓글의 내용을 수정할 수 있는 입력 필드로 변경
        var commentDiv = document.getElementById('commentContent' + commentId);
        var content = commentDiv.innerHTML;
        commentDiv.innerHTML = '<input id="editedComment" value="' + content + '" />';
        
        // 확인 버튼을 추가하여 수정 내용을 저장하도록 함
        var saveButton = document.createElement('button');
        saveButton.innerHTML = '확인';
        saveButton.onclick = function() {
            var editedContent = document.getElementById('editedComment').value;
            // AJAX 등을 사용하여 수정된 내용을 서버에 전달하고 업데이트할 수 있음
            // 이 예시에서는 수정된 내용을 commentupdate.jsp로 전송함
            var form = document.createElement('form');
            form.method = 'post';
            form.action = 'CommentUpdate.jsp';
            ...
            var editedContentInput = document.createElement('input');
            editedContentInput.type = 'hidden';
            editedContentInput.name = 'content';
            editedContentInput.value = editedContent;
            form.appendChild(editedContentInput);
